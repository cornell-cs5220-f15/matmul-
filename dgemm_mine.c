...
void matrix_copy (const int mat_size, const int sub_size, const int i, const int j,
  ...
  const int M = ((i+1)*sub_size > mat_size? mat_size-i : sub_size); // Maybe we can do this outside, but I'm not worried about this right now.
  const int N = ((j+1)*sub_size > mat_size? mat_size-j : sub_size);
  // Make a copy
  int m, n;
  for (m = 0; m < M; m++){
    for (n = 0; n < N; n++){
