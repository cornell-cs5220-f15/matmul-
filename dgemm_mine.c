...
void square_dgemm(const int M, const double *A, const double *B, double *C)
{
    ...
    double *a_transposed = (double*) _mm_malloc(M * M * sizeof(double), 32);
    transpose_array(M, A, a_transposed);

    int malloc_block_size = BLOCK_SIZE * BLOCK_SIZE * sizeof(double);

    double* a_block = (double*) _mm_malloc(malloc_block_size, 32);
    double* b_block = (double*) _mm_malloc(malloc_block_size, 32);
    double* c_block = (double*) _mm_malloc(malloc_block_size, 32);
