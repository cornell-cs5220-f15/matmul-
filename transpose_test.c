...
void test_cm_transpose_into(void);

int main(void) {
    test_cm_transpose();
    test_rm_transpose();
    test_cm_transpose_into();
...
void test_cm_transpose_into(void) {
    ...
    cm_transpose_into(&A[cm(lN, lM, 1, 1)], lN, lM, N, M, A_, A_N, A_M);

    assert (A_[rm(A_N, A_M, 0, 0)] == 0);
    assert (A_[rm(A_N, A_M, 0, 1)] == 1);
    assert (A_[rm(A_N, A_M, 0, 2)] == 2);
    assert (A_[rm(A_N, A_M, 0, 3)] == 3);
    assert (A_[rm(A_N, A_M, 1, 0)] == 4);
    assert (A_[rm(A_N, A_M, 1, 1)] == 5);
    assert (A_[rm(A_N, A_M, 1, 2)] == 6);
    assert (A_[rm(A_N, A_M, 1, 3)] == 7);
    assert (A_[rm(A_N, A_M, 2, 0)] == 8);
    assert (A_[rm(A_N, A_M, 2, 1)] == 9);
    assert (A_[rm(A_N, A_M, 2, 2)] == 10);
    assert (A_[rm(A_N, A_M, 2, 3)] == 11);
}
